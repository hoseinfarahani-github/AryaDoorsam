...
class DashboardController extends FactoryController
{
    ...
    {
        $status=[];
        $pending_order_count=Order::whereIn('status',['approved_order_manager','pending_order_factory'])->count();
        $approved_order_count=Order::where('status','approved-order-factory')->count();
        ...
        $status['سفارش در حال بررسی']=$pending_order_count;
        $status['سفارش تایید شده']=$approved_order_count;
