...
class DashboardController extends AgentController
{
    ...
    public function pie_report()
    {
        ...
        $draft_order_count=Auth::user()->agent->order
            ->whereIn('status',['create-order-step-1','create-order-step-2','create-order-step-3','sent-order-agent','pending-order-CEO'])->count();
        $rejected_order_count=Auth::user()->agent->order
            ->where('status','reject-order-CEO')->count();
        $approved_order_count=Auth::user()->agent->order->count()-($rejected_order_count+$draft_order_count);
    ...
    {
		//dd(Auth::user()->agent->order->toQuery()->latest()->take(4)->get());
       // dd(Auth::user()->order[1]->order_detail);
        ...
        $user_best_selling_product=Product::all()->take(3);
		        $user_order=new Collection();
        if(Auth::user()->agent->order->count()!= 0){
            $user_order=Auth::user()->agent->order->toQuery()->latest()->take(4)->get();
        }

        $product_count=Product::all()->count();
        $user_order_count=Auth::user()->agent->order->count();
        $user_client_count=Auth::user()->agent->client->count();
