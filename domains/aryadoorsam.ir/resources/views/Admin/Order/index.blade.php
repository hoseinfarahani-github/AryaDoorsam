...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            @can('create-product')
                <a href="{{route('admin.client.create')}}" class="button text-white bg-theme-1 shadow-md ml-2">افزودن سفارش جدید</a>
            @endcan
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <table class="table table-report sm:mt-2">
                        ...
                        @foreach($orders  as $order)
                            ...
                            {{--                        <form method="get" action="{{route('products.edit',$order->id)}}">--}}
                            {{--                            @csrf--}}
                            <tr class="intro-x">
                                <td class="w-40">
                                    <div class="flex justify-center items-center">
                                        <a href="{{route('admin.order.show',$order)}}" class="flex items-center ml-3" > <i data-feather="eye" class="w-4 h-4 ml-1 "></i> </a>

                                            <a href="{{route('admin.order.edit',$order->id)}}" class="flex items-center ml-3" > <i data-feather="check-square" class="w-4 h-4 ml-1 "></i> </a>

                                        <a data-toggle="modal" id="submit" data-target="#UserDelete{{$order->id}}" class="flex items-center text-theme-6" href="javascript:;">
                                                <i data-feather="trash-2" class="w-4 h-4 ml-1"></i> </a>
                                    </div>
                                </td>

                                <td class="text-center">{{__('status.'. $order->status)}} </td>
                                <td class="text-center">
                                     <a class="button w-24 rounded-full mr-1 mb-2  bg-theme-2 bg-theme-8 text-black text-xs">{{$order->tracking_serial??'ندارد'}}</a>
                                </td>

                                <td class="table-report__action w-56">
                                    <div class="flex w-10 m-auto">
                                        <div class="text-gray-600 text-xs whitespace-no-wrap flex-justify-start">  {{__('users.'. $order->agent->user->username)}}</div>
                                    </div>
                                </td>
                                <td>
                                    <a href="" class="font-medium whitespace-no-wrap flex-justify-start">{{$order->client->name()}}</a>
                                    <div class="text-gray-600 text-xs whitespace-no-wrap flex-justify-start"><a href="tel:"></a> </div>
                                </td>
                                <td>
                                    <a href="" class="font-medium whitespace-no-wrap flex-justify-start text-center">{{$order->id}}</a>
                                </td>
                                <td class="table-report__action w-5">
                                    @can('delete-product')
                                        <input onclick="multidelete(this);" value="{{$order->productname}}" name="{{$order->id}}" class="marked input border border-gray-500" type="checkbox">
                                    @endcan
                                </td>
                            </tr>
                            {{--                        </form>--}}
        ...
        <div class="mt-5 flex-justify-profile intro-y col-span-12 sm:flex-row sm:flex-no-wrap items-center ">
            {{--                {{$brands->links()}}--}}
{{--            @include('Admin.Pagination.index', ['paginator' => $orders,'route' => 'admin.client'])--}}
        </div>
        <!-- END: Pagination -->
    </div>
                ...
                <div class="modal__content">
                    <form action="{{route('admin.order.destroy',$order)}}" method="post" >
                        ...
                        <div class="p-5 text-center"> <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                            <div class="text-3xl mt-5">حذف سفارش  <span style="color: #8b0000"></span></div>
                            {{--                        <div class="mt-3 ">--}}
