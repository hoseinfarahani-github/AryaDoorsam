...
class DashboardController extends ManagerController
{
    ...
    public function pie_report()
    {
        ...
        $status['در انتظار تایید مدیر']=$new_created_order;
		$status['لغو شده توسط مدیر']=$rejected_order;
        $status['تایید شده توسط مدیر']=$confirm_order;
        return $status;
    }
    ...
    {
        $user_order=Order::query()->whereNotIn('status',['create-order-step-1','create-order-step-2','create-order-step-3'])->latest()->take(4)->get();
		$user_order_count=Order::query()->whereNotIn('status',['create-order-step-1','create-order-step-2','create-order-step-3'])->get()->count();
