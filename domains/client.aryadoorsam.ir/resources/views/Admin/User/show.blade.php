        ...
        <div class="content">
            ...
            <div class="intro-y box px-5 pt-5 mt-5">
                <div class="flex flex-col lg:flex-row border-b border-gray-200 pb-5 -mx-5">
                    <div class="flex flex-1 px-5 items-center justify-center lg:justify-end">
                        <div class="ml-5">
                            <div class="w-24 sm:w-40 truncate sm:whitespace-normal font-medium text-lg mr-2">{{$user->username}}</div>
{{--
                            <a href="tel:{{$user->phone}}" class="text-gray-600 mr-2">{{$user->phone}}</a>
--}}
                            <a class="text-gray-600 mr-2">
                                @if($user->is_haghighi())(حقیقی)
                                @elseif($user->is_hoghoghi()) (حقوقی)
                                @else() (احراز نشده)
                                @endif
                            </a>
                        </div>
                    ...
                    <div class="flex mt-6 lg:mt-0 items-center lg:items-start flex-1 flex-col justify-center text-gray-600 px-5 border-l border-r border-gray-200 border-t lg:border-t-0 pt-5 lg:pt-0">
                        <a class="truncate sm:whitespace-normal flex items-center"> {{$user->email}}  <i data-feather="mail" class="w-4 h-4 ml-2"></i></a>
                        <a href="tel:{{$user->phone}}"  class="truncate sm:whitespace-normal flex items-center mt-3">{{$user->phone}} <i data-feather="phone" class="w-4 h-4 ml-2"></i></a>

                        {{--

                        <a class="truncate sm:whitespace-normal flex items-center mt-3 {{(AuthenticationTo($user->authentication)['text'])}}">{{(AuthenticationTo($user->authentication)['persian'])}}<i data-feather="flag" class="w-4 h-4 ml-2"></i>  </a>
--}}
                    </div>
                    <div class="mt-6 lg:mt-0 flex-1 px-5 border-t lg:border-0 border-gray-200 pt-5 lg:pt-0">
                        <div class="flex items-center justify-center lg:justify-end">

                        <div id="authenticationDropDown" class="dropdown relative mt-2 item-center">
                            <button  class="dropdown-toggle button {{AuthenticationTo($user->authentication['eng'])['theme']}} inline-block  text-white">{{AuthenticationTo($user->authentication['eng'])['persian']}}</button>
                            <div class="dropdown-box mt-10 absolute w-64 top-0 right-0 z-20">
                                <div class="dropdown-box__content box p-2">
                                    {{--bg-theme-5--}}


                                    @foreach($statusAuthentication as $sa)
                                        <form action="{{route('admin.user.changeAuthentication',$user)}}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <input name="authenticationStatus" value="{{$sa}}"  hidden >
                                        <button type="submit"    class=" {{AuthenticationTo($sa)['text']}}  @if($sa == $user->authentication) bg-theme-8 @endif  flex rtl  items-center block p-2 transition duration-300 ease-in-out bg-white hover:bg-gray-200 rounded-md">
                                        {{AuthenticationTo($sa)['persian']}}</button>
                                    </form>
                ...
                <div class="nav-tabs flex flex-col sm:flex-row justify-center lg:justify-start">
                    <a data-toggle="tab" data-target="#dashboard" href="javascript:;" class="py-4 sm:mr-8 active">داشبور</a>
                    <a data-toggle="tab" data-target="#account-and-profile" href="javascript:;" class="py-4 sm:mr-8">پروفایل</a>
                    <a data-toggle="tab" data-target="#activities" href="javascript:;" class="py-4 sm:mr-8">فعالیت</a>
                    <a data-toggle="tab" data-target="#orderCredit" href="javascript:;" class="py-4 sm:mr-8">اعتبار و خرید</a>
                    <a data-toggle="tab" data-target="#rollPermission" href="javascript:;" class="py-4 sm:mr-8">نقش و مقام</a>
                    <a data-toggle="tab" data-target="#documents" href="javascript:;" class="py-4 sm:mr-8">مدارک</a> </div>
            </div>
